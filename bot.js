  ...
  return {
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: lang === 'ua' ? '✅ Я виконав всі умови!' : '✅ Я выполнил все условия!', callback_data: `request_${guideKey}` }],
...
Привіт! Мене звати Юлія Хацевич... (твій текст без змін)`,
...
Для консультацій та питань звертайтесь у Direct Instagram або до цього бота.`,
    ...
    // Эти строки у тебя используются, но в коде их не было — добавляю
...
Привет! Меня зовут Юлия Хацевич... (твій текст без змін)`,
...
async function checkBasicInstagramConditions(username) {
  // В твоем коде это уже было, просто оставляю как "успех"
  return { success: true };
}
...
bot.on('callback_query', async (callbackQuery) => {
  ...
  try {
    ...
    } else if (data.startsWith('guide_')) {
      const guideKey = data.split('_')[1];
      const guide = GUIDES[guideKey];
